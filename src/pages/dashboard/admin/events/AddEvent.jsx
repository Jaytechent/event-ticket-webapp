...
const AddEvent = () => {
    ...
    const onSubmit = async (data) => {
        const { title, type, location, speakers, sponsor,description, seat, image, price,date } = data;
        const imageResult = await uploadImage(image[0]);
        const  loadingToast = toast.loading('Creating New Event..');
        if (imageResult) {
            const newEvent = { title, type, location, speakers, sponsor,description, seat, image:imageResult, date, price }
