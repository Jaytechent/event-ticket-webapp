...
//        `${import.meta.env.VITE_API_BASE_URL}/event/${params._id}`
//     );
//     if (!response.ok) {
...
const PaymentPage = () => {
  const params = useParams();
  const [ticketPrice, setTicketPrice] = useState(1000); // Default price for ticket option 1
  const [addMoreTicket, setAddMoreTicket] = useState(1);
  
 
  const taxes = ticketPrice * 0.10; // 10% tax
  const totalPrice = (ticketPrice + taxes) * addMoreTicket; // Total price including taxes
  const axios = useAxiosPublic();
  const { user } = useAuth();
const currentticketPrice = ticketPrice * addMoreTicket
const currenttickettax = taxes * addMoreTicket
  const ticketPriceHandle = () => {
    if (addMoreTicket === 1) {
      setTicketPrice(1000); 
    }
  ...
  const ticketPriceHandle2 = () => {
    if (addMoreTicket === 1) {
      setTicketPrice(2000); // Set ticket price to 2000
    }
  };
  ...
  const handleAddMoreTicket = () => {
    if (addMoreTicket < 10) {
      ...
      Swal.fire({
        title: "Oops!",
        text: "You can't purchase more than 10 tickets",
        icon: "error"
      });
    }
  };
  ...
  return (
    <div className='max-w-screen-xl mx-auto'>
      <div className='grid md:grid-cols-3 px-4 md:px-1 lg:gap-4'>
        <div className='col-span-2'>
          <PricingCards 
            ticketPriceHandle={ticketPriceHandle} 
            ticketPriceHandle2={ticketPriceHandle2} 
            isTicketOneActive={addMoreTicket === 1} // Determine if ticket 1 is active
            isTicketTwoActive={addMoreTicket === 1} // Determine if ticket 2 is active
          />
        </div>
        <div className='mt-16 col-span-2 md:col-span-1'>
          <h2 className='text-2xl text-center font-semibold'>Payment details</h2>
          <div className='bg-slate-100 rounded-md border-2 border-black mt-7 p-5 pb-12'>
            ...
            <div className='flex md:flex-col lg:flex-row gap-3 lg:gap-5 justify-between mt-4'>
              ...
              <h2 className='font-semibold text-xl'>#{currentticketPrice}</h2>
            </div>
            <div className='mt-5'>
              <h2 className='flex items-center gap-2'><MdOutlineCalendarMonth />{formattedDate}</h2>
              <h2 className='flex items-center gap-2 mt-2'><CiLocationOn />{event.location}</h2>
            ...
            <div className='flex mt-4 justify-between items-center'>
              <h2>Taxes</h2>
              <p className='font-semibold'>#{currenttickettax}</p>
            </div>
            <hr className='h-[1.6px] mt-1 bg-gray-500' />
            <div className='flex mt-4 justify-between items-center'>
              <h2 className='text-2xl font-semibold'>Total</h2>
            ...
            <div className='mt-4 flex gap-2'>
              <h2 className='text-lg text-gray-700 font-semibold'>Buy more tickets</h2>
              <div className='flex gap-1'>
                <CiSquareMinus onClick={handleRemoveMoreTicket} className='text-3xl' />
